...
def get_telnet_shell_by_uri(**kwargs):
    ...
    if hostname and username and password and port:
        shell = Shell(
            "telnet://%s:%s@%s:%d" % (username, quote_plus(password), hostname, port),
            **kwargs
        )
...
def get_secureshell_by_uri(**kwargs):
    ...
    if hostname and username and password and port:
        shell = Shell(
            "ssh://%s:%s@%s:%d" % (username, quote_plus(password), hostname, port),
            **kwargs
        )
...
def get_serialshell_by_uri(**kwargs):
    ...
    if username and password:
        return Shell(
            "serial://%s:%s@%d?baudrate=%d" % (username, password, port, baudrate),
            **kwargs
        )
    else:
